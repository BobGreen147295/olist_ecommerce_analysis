...
type ReactivationSignal = { state: "ready" | "not_ready"; message: string; eligible_customers?: number; eligible_historical_sales?: number; currency?: string; cutoff_date?: string; inactive_days?: number };
const evidenceById = {
...
export default function OpportunitiesPage() {
  ...
  return <main className="page-content">
    ...
    {reactivationSignal && <section className="card aggregate-opportunity-card"><div><p className="eyebrow">CONSENTED REACTIVATION PILOT</p><h2>高价值沉默客户：{reactivationSignal.state === "ready" ? "待审核机会" : "数据尚未就绪"}</h2><p>{reactivationSignal.message}</p></div><div className="aggregate-signal-list"><article>{reactivationSignal.state === "ready" ? <><strong>{reactivationSignal.eligible_customers?.toLocaleString() ?? 0} 位可进入试点的客户</strong><span>历史消费额：{reactivationSignal.currency} {reactivationSignal.eligible_historical_sales?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span><small>规则：已同意营销、消费前 25%、相对最新订单沉默 {reactivationSignal.inactive_days} 天；截止 {reactivationSignal.cutoff_date}。</small></> : <><strong>尚不能计算真实客户机会</strong><small>需要在数据页导入匿名稳定客户 ID 和营销同意状态；不会接受邮箱、电话或名单。</small></>}</article></div></section>}

...
export default function OverviewPage() {
  ...
  return <main className="page-content overview-page">
    ...
    <section className="content-grid overview-grid"><article className="card opportunity-hero"><div className="decision-index"><span>NEXT BEST ACTION</span><strong>01</strong></div><div className="decision-body"><div className="card-kicker"><StatusBadge tone="warning">{top.priority} 优先级</StatusBadge><span>合成演示建议 · 需要你确认</span></div><h2>{top.title}</h2><p className="hero-copy">{top.evidence}。先用小预算测试，确认增量后再扩大人群与渠道。</p><div className="signal-row"><div><span>目标人群</span><strong>{top.segment}</strong></div><div><span>30 天机会</span><strong>{top.potential}</strong></div><div><span>综合分</span><strong>{top.score}/100</strong></div></div><div className="button-row"><Link className="button decision-button" href="/campaigns">查看活动草案 <span aria-hidden>→</span></Link><Link className="button decision-link" href="/opportunities">审查证据</Link></div></div></article><article className="card readiness-card"><div className="card-kicker"><span>DATA READINESS</span><StatusBadge tone={realOpportunityReady ? "success" : "warning"}>{realOpportunityReady ? "MODEL READY" : "DATA GATE"}</StatusBadge></div><h3>真实机会建模状态</h3><div className="readiness-visual"><div className="readiness-score">{summary ? "74" : "68"}<span>/100</span></div><span>已完成基础连接</span></div><p>{realOpportunityGate}</p><Link href="/data" className="text-link">查看数据连接清单 →</Link></article></section>

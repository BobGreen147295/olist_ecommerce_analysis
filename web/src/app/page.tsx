...
export default function OverviewPage() {
  ...
  return <main className="page-content overview-page">
    ...
    <section className="notice-bar"><span className="notice-dot" />{summary ? isDevelopmentStore ? (english ? `Shopify development-store aggregates connected: ${shopify?.shop_domain}. Used only to validate sync; real opportunities and customer activation remain locked.` : `已接入 Shopify 开发店汇总数据：${shopify?.shop_domain}。仅用于同步验证，不会解锁真实机会或客户触达。`) : (english ? `Authorized Shopify aggregates connected: ${shopify?.shop_domain}. Counts below are real; the opportunity queue remains explicitly synthetic and is never mixed with them.` : `已接入 Shopify 授权汇总数据：${shopify?.shop_domain}。以下计数为真实数据；机会队列仍明确标为合成演示场景，二者不会混用。`) : (english ? `This is a demo workspace using ${workspace.dataSource}. No messages are sent to real customers.` : `当前是${workspace.mode}：展示数据来自 ${workspace.dataSource}，不会向任何真实客户发送触达。`)}</section>
    ...
    <section className="card table-card"><div className="table-head"><span>{t("opportunity")}</span><span>{t("owner")}</span><span>{t("estimatedOpportunity")}</span><span>{t("compositeScore")}</span><span>{t("status")}</span></div>{opportunities.map((item) => <div className="table-row" key={item.id}><div><strong>{item.title}</strong><small>{item.segment} · {t("demoLabel")}</small></div><span>{item.owner}</span><strong>{item.potential}</strong><strong>{item.score}</strong><StatusBadge tone={item.status === "待审批" ? "warning" : "neutral"}>{item.status}</StatusBadge></div>)}</section>
  </main>;
}

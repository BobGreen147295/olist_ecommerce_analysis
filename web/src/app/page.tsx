...
export default function OverviewPage() {
  ...
  const realOpportunityReady = Boolean(trend && trend.orders_scanned >= 20 && trend.days.length >= 3);
  const realOpportunityGate = !trend ? "尚未同步订单趋势，不能评估真实机会。" : realOpportunityReady ? "已满足基础数据门槛；确认分析口径后可进入真实机会建模。" : `当前仅 ${trend.orders_scanned} 笔订单、${trend.days.length} 个有订单的日期；达到 20 笔订单且覆盖 3 个日期后，才可进入真实机会建模。`;

...
export default function OverviewPage() {
  ...
  return <main className="page-content">
    ...
    {trend && <section className="card api-card"><div><p className="eyebrow">SHOPIFY ORDER TREND</p><h2>近 {trend.window_days} 天订单汇总</h2><p>订单 {trend.totals.orders} 笔 · 销售额 {money(trend.totals.gross_sales)} · 退款额 {money(trend.totals.refunds)} · 净销售额 {money(trend.totals.net_sales)}。仅保存按日汇总，退款{trend.refund_attribution}{trend.truncated ? "；已达到读取上限，结果可能不完整" : "。"}</p></div><span className="api-tag">{trend.orders_scanned} ORDERS AGGREGATED</span></section>}

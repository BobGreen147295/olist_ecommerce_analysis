...
export default function OverviewPage() {
  ...
  return <main className="page-content">
    ...
    {summary ? <section className="metrics-grid"><Metric label="Shopify 订单" value={summary.orders.toLocaleString()} detail="授权店铺聚合数量" trend="不保存订单明细" /><Metric label="Shopify 客户" value={summary.customers.toLocaleString()} detail="授权店铺聚合数量" trend="不保存客户身份信息" /><Metric label="Shopify 产品" value={summary.products.toLocaleString()} detail="授权店铺聚合数量" trend="只读汇总同步" /><Metric label="库存项" value={summary.inventory_items.toLocaleString()} detail="最多读取 250 个库存项" trend={summary.currency_code ? `店铺币种 ${summary.currency_code}` : "未返回币种"} /></section> : <section className="metrics-grid"><Metric label="可验证收入机会" value="$38.1k" detail="未来 30 天估算上限" trend="3 个待处理机会" /><Metric label="待人工审批" value="2" detail="Agent 已生成完整方案" trend="需确认人群与预算" /><Metric label="进行中的实验" value="1" detail="新客第二单激励" trend="第 4 / 14 天" /><Metric label="已验证净增收入" value="$6.8k" detail="最近 30 天（示例）" trend="基于对照组归因" /></section>}
